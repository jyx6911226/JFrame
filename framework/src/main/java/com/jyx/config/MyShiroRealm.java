...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		List<SearchFilter> roleFilters = new ArrayList<>();
		SearchFilter userFilter = new SearchFilter("userInfos.id", SearchFilter.Operator.EQ, userInfo.getId(), SearchFilter.Connector.AND);
		roleFilters.add(userFilter);
		List<SysRole> roleList = null;
		try {
			roleList = sysRoleService.findEntityPage(null,null,roleFilters).getContent();
		} catch (Exception e) {
			e.printStackTrace();
		}
